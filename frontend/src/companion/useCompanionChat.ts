...
export const useCompanionChat = (): CompanionChatApi => {
  const historyRef = useRef<Array<{ role: string; content: string }>>([]);

  const send = useCallback(async (text: string, opts?: { mode?: string; conceptId?: string | null }) => {
  ...
  const answerQuiz = useCallback(async (messageId: string, optionIndex: number) => {
    const store = useCompanionStore.getState();
  ...
  const quickActions = useCallback((): CompanionAction[] => {
    const snap = useCompanionStore.getState().snapshot;
    return snap ? composerActions(snap) : [];
  }, []);

  return { send, runAction, answerQuiz, greet, quickActions };
};
